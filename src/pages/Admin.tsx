...
export function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<'import' | 'export' | 'create' | 'bulk' | 'audit'>(
    'import',
  );
    ...
    reader.onload = e => {
      try {
        ...
        const dataRows = raw
          ...
          .map(row =>
            Object.fromEntries(hdrs.map((h, i) => [h, (row as unknown[])[i] ?? ''])),
          );
        setHeaders(hdrs);
        setRows(dataRows);
      } catch {
...
function ExportTab() {
  ...
  const handleExport = () => {
    ...
    DATASETS.filter(d => selected.has(d.key)).forEach(dataset => {
      const data = dataset.fn() as object[];
      if (data.length === 0) {
