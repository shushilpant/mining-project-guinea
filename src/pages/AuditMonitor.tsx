...
import {
  ...
  FileInput, Users, AlertTriangle, GitCommit, Settings,
  CheckCircle, Flag, ArrowUpCircle, RotateCcw, AlertCircle,
...
const ACTION_CFG: Record<AuditAction, { label: string; color: string; bg: string }> = {
...
const ENTITY_ICON: Record<AuditEntity, React.ElementType> = {
  agreement:          FileInput,
  operator:           Users,
...
const REVIEW_CFG: Record<ReviewStatus, { label: string; color: string; bg: string; border: string }> = {
...
// ─── Review controls (inside expanded table row) ──────────────────────────────

function ReviewControls({ entry }: { entry: AuditEntry }) {
...
function EntityBreakdown({ entries }: { entries: AuditEntry[] }) {
